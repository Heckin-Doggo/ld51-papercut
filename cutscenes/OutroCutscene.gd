...
func play_cutscene():
	...
	Slide.texture = load("res://assets/art/cutscene/EndCharger.jpg")
	dialog = dialogBox.instance()
	dialog.dialogPath = "res://dialog/outroCutscene1.json"
	$CanvasLayer.add_child(dialog)
	yield(dialog, "finished")
	
	Slide.texture = load("res://assets/art/cutscene/EndCourthouse.jpg")
	dialog = dialogBox.instance()
	dialog.dialogPath = "res://dialog/outroCutscene2.json"
	$CanvasLayer.add_child(dialog)
	yield(dialog, "finished")
